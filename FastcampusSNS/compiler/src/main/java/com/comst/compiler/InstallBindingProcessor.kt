...
class InstallBindingProcessor : AbstractProcessor() {
    ...
    private fun processElement(typeElement: TypeElement) {
        ...
        val annotationProperties = ElementFilter.methodsIn(annotationTypeElement.enclosedElements)
            .associateBy { it.simpleName.toString() }
        val component = (annotation.elementValues[annotationProperties["component"]]?.value
            ?: error("component 값을 찾을 수 없습니다")) as DeclaredType
